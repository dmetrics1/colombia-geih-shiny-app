...
ui <- dashboardPage(
  ...
  dashboardBody(
    tags$head(
      ...
      tags$link(rel = "stylesheet", type = "text/css", href = "brand.css?v=16"),
      tags$script(HTML(paste(
        ...
        sep = "\n"))),
      # Reveal on scroll: las tarjetas/gráficos entran desde abajo al hacer scroll
      tags$script(HTML(paste(
        "(function(){",
        ...
        "  function scan(){",
        "    document.querySelectorAll('.content-wrapper .card-panel:not(.reveal-init), .content-wrapper .kpi-row:not(.reveal-init)').forEach(function(el){",
        "      el.classList.add('reveal-on-scroll'); el.classList.add('reveal-init');",
        ...
        "      if(r.top < (window.innerHeight - 24) && r.bottom > 0){ el.classList.add('revealed'); }",
        "    });",
        "  }",
        "  if(document.readyState !== 'loading'){ scan(); } else { document.addEventListener('DOMContentLoaded', scan); }",
        "  $(document).on('shiny:idle', function(){ setTimeout(scan, 40); });",
        "  $(document).on('click', '.sidebar-menu a', function(){ setTimeout(function(){ scan(); revealInView(); }, 160); });",
        "})();",
        sep = "\n")))
    ),
